(function(namespace) {
    ...
    Game.prototype.checkCactusHit = function() {
        for (var i = 0; i < this.cacti.length; i++) {
            console.log('Checking collision with cactus', i, this.cacti[i]);
            if (this.player.collidesWith(this.cacti[i], this.offset)) {
                console.log('Collision detected with cactus', i);
                this.running = false;
                this.finished = true;
    ...
    // Add event listener for space bar to reload the game if finished
    document.addEventListener('keydown', function(event) {
        if (event.code === 'Space' && game.finished) {
            location.reload();
        }
    });
